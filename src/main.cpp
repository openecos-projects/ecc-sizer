...
int main(int argc, char *argv[]) {
  ...
  string benchmarkName, dirRoot, pdkDir;
  ...
  if (app.hasOption("report")) {
    ...
    if (flow == "tiago")
      ...
      flowIsvlsi13(benchmarkName, dirRoot, pdkDir);
    else if (flow == "flow0")
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
std::vector<string> get_all_files_in_dir(const std::string &dir_path) {
...
void flowIsvlsi13(string benchmarkName, string dirRoot, string dirPDK) {
  Circuit myCircuit;

  cout << benchmarkName << endl;

  std::string lef_dir = dirPDK + "/lef/";
  std::string lib_dir = dirPDK + "/lib/";
  std::string tech_File = lef_dir + "asap7_tech_1x_201209.lef";

  std::vector<std::string> libFiles = get_all_files_in_dir(lib_dir);
  std::vector<std::string> lefFiles = get_all_files_in_dir(lef_dir);
  std::string verlogFile =
      dirRoot + "/" + benchmarkName + "/" + benchmarkName + ".v";
  std::string sepfFile =
      dirRoot + "/" + benchmarkName + "/" + benchmarkName + ".sepf";
  std::string sdcFile =
      dirRoot + "/" + benchmarkName + "/" + benchmarkName + ".sdc";

  // myCircuit.benchName = benchmarkName;
  // myCircuit.rootDir = dirRoot;
  myCircuit.initFilePath(libFiles, lefFiles, verlogFile, sepfFile, sdcFile);
  myCircuit.readInputFiles();
