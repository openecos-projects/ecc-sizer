...
// special characters The return value indicates success/failure.
...
bool VerilogParser::read_cell_inst(
...
bool SdcParser::read_clock(string &clockName, string &clockPort,
                           double &period) {
...
// Return value indicates if the last read was successful or not.
bool SdcParser::read_driver_info(string &inPortName, string &driverSize,
...
// Return value indicates if the last read was successful or not.
bool TimingParser::read_port_timing(string &port, double &riseSlack,
...
void LibParser::_begin_read_timing_info(string toPin,
                                        LibParserTimingInfo &timing) {
...
void LibParser::_begin_read_pin_info(string pinName, LibParserCellInfo &cell,
  ...
  while (!finishedReading) {
    ...
    if (tokens[0] == "direction") {
      ...
      cell.timingArcs.push_back(
          LibParserTimingInfo()); // add an empty TimingInfo object
      _begin_read_timing_info(
          pinName,
          cell.timingArcs
              .back()); // pass the empty object to the function to be filled
...
void LibParser::_begin_read_cell_info(string cellName,
  ...
  while (!finishedReading) {
    ...
    if (tokens[0] == "cell_leakage_power") {
      ...
      _begin_read_pin_info(
          tokens[1], cell,
          cell.pins.back()); // pass the new PinInfo object to be filled
...
ostream &operator<<(ostream &os, LibParserLUT &lut) {
  ...
  assert(!lut.loadIndices.empty() && !lut.transitionIndices.empty() &&
         !lut.tableVals.empty());
...
// Example function that uses VerilogParser class to parse the given ISPD-12
// verilog file. The extracted data is simply printed out in this example.
...
// Example function that uses TimingParser class to parse the given ISPD-12
// timing file. The extracted data is simply printed out in this example.
void test_timing_parser(string filename) {
  ...
  do {
    ...
    if (valid) {
      std::cout << "Timing for port: " << port << ": " << riseSlack << " "
