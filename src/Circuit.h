...
#define MAKE_OPERATOR(OP)                                                      \
  friend EdgeArray<T> operator OP(const EdgeArray<T> v0,                       \
    ...
    return EdgeArray<T>(v0[RISE] OP v1[RISE], v0[FALL] OP v1[FALL]);           \
  }                                                                            \
...
public:
  ...
  void initFilePath(std::vector<string> libFiles, std::vector<string> lefFiles,
                    std::string verlogFile, std::string sepfFile,std::string sdcFile) {
    this->libFiles = libFiles;
    this->lefFiles = lefFiles;
    this->verlogFile = verlogFile;
    this->sepfFile = sepfFile;
    this->sdcFile = sdcFile;
  }
  std::vector<string> libFiles;
  std::vector<string> lefFiles;
  std::string verlogFile;
  std::string sepfFile;
  std::string sdcFile;
  // std::vector<string> libFiles;
private:
  template <typename T> class EdgeArray {
    ...
    friend EdgeArray<T> max(const EdgeArray<T> v0, const EdgeArray<T> v1) {
      return EdgeArray<T>(max(v0[RISE], v1[RISE]), max(v0[FALL], v1[FALL]));
    }
    friend EdgeArray<T> min(const EdgeArray<T> v0, const EdgeArray<T> v1) {
      return EdgeArray<T>(min(v0[RISE], v1[RISE]), min(v0[FALL], v1[FALL]));
    }

    friend EdgeArray<T> abs(const EdgeArray<T> v) {
      return EdgeArray<T>(fabs(v[RISE]), fabs(v[FALL]));
    }
    friend EdgeArray<T> pow(const EdgeArray<T> v, const double exp) {
      return EdgeArray<T>(pow(v[RISE], exp), pow(v[FALL], exp));
    }
    friend EdgeArray<T> sqrt(const EdgeArray<T> v) {
      return EdgeArray<T>(sqrt(v[RISE]), sqrt(v[FALL]));
    }
...
private:
  ...
  struct LogicalEffort {
    ...
    LogicalEffort()
        : cin(0, 0), g(0, 0), p(0, 0), valid(0, 0), residuum(0, 0) {
    } // end constructor
  };  // end struct

  struct LibArcCharacterization : LogicalEffort {}; // end struct
  ...
  LibParserCellInfo &getLibCell(const int footprint, const int vth,
                                const int size) {
    return orgCells.oCells[footprint]
        .cells[timingLibraryCharacterization[footprint]
                   .mappingVthSizeToCellTypeIndex[vth][size]];
  } // end method

  LibCellCharacterization &getLibCellCharacterization(const int footprint,
                                                      const int type) {
  ...
  LibCellCharacterization &getLibCellCharacterization(Vcell *cell) {
    return getLibCellCharacterization(cell->footprintIndex,
  ...
  struct TimingNet {
    Vcell *driver;
    int fanout; // number of timing arcs driven by this net (used in the slew
  ...
  struct TimingArcState {
    ...
    // Reimann
    EdgeArray<double>
        slack; // slack (m_u->v) (worst arrival time + arc delay + worst delay
               // to endpoint) [TODO] consider moving to outside

    // Logical Effort
    EdgeArray<LogicalEffortPath> logicalEffortPath;
    double logicalEffortCin;
    EdgeArray<double>
        logicalEffortArrivalTime; // arrival time at the input of this arc
  ...
  struct TimingNetState {
    double
        load; // total net load (due to wires, input pins and primary outputs)
    EdgeArray<double> slew;        // slew in this net
    EdgeArray<double> arrivalTime; // arrival time at this net
    EdgeArray<int> backtrack;      // index of net driving the driver input with
                                   // greatest arrival time
    EdgeArray<int> backtrackSlew;  // index of net driving the driver input with
                                   // greatest slew
    ...
    // Logical Effort
    EdgeArray<LogicalEffortPath> logicalEffortPath;
    EdgeArray<int> logicalEffortBacktrack; // direct index of the timing arc
                                           // that propagated the logica effort
    double logicalEffortLoad;
    EdgeArray<double>
        logicalEffortArrivalTime; // logical effort arrival time at this net
  ...
  int timingOffsetToLevelOneNets; // skip dummy and level=0 (primary inputs and
                                  // ffs driven nets)
  ...
  void updateTiming_Nets(const int n0, const int n1,
                         const int threadId = 0); // [n0,n1)
  ...
  double updateCellAndComputeSizingEffectOnDriverCellDelay(Vcell *cell,
                                                           const int newSize);
  ...
  vector<pair<string, double>>
      footprintMinimumSlewPairs; // footprint and logical effort
  ...
  public:
    void setup(Circuit *circuit, const int n0, const int n1,
               const int threadId) {
  ...
  double computeLogicalEffortDelay(const int k, const double G, const double B,
...
private:
  double logicalEffort_GetWns() const {
    return getT() - logicalEffortWorstArrivalTime.getMax();
  }

  double logicalEffort_GetGain(const TimingNetState &netstate,
                               const TimingArcState &arcstate) const {
    return netstate.logicalEffortLoad / (arcstate.logicalEffortCin > 0
...
public:
  ...
  double maxTransition, worstSlack, worstSlew, worstDelay, worstNSDelay,
      avgDelay, minSlack, minSlew;
  bool hasViol;
  ...
  vector<EdgeArray<double>> deltaDelay_deltaLoad, deltaSlew_deltaLoad,
      deltaDelay_deltaSlew;
  ...
  // Build circuit graph and set initial solution (false = min leakage, true =
  // random solution)
  ...
  bool updateCellTypeLagrangeRelaxationSensitivities(Vcell *cell,
                                                     const double gamma,
                                                     const double alpha = 1.0);
  bool updateCellTypeLagrangeRelaxationSensitivitiesTestingChanges(
      Vcell *cell, const double gamma, const double alpha = 1.0);
  bool updateCellTypeConsideringLocalSlack(Vcell *cell,
                                           const double alpha = 1.0);
  ...
  void computeLogicalEffort_LinearLeastSquares(const vector<double> &x,
                                               const vector<double> &y,
                                               double &a, double &b);
  void computeLogicalEffort_LinearLeastSquaresError(const vector<double> &x,
                                                    const vector<double> &y,
                                                    const double a,
                                                    const double b,
                                                    double &residuum);
  void computeLogicalEffort_TimingArc(const LibParserLUT &lut, const double Cin,
                                      const double inputSlew,
                                      const vector<double> &gains, double &g,
                                      double &p, double &residuum);
  void computeLogicalEffort_Report(ostream &out);
  ...
  double computeFanoutFaninRatio(const LibParserCellInfo &cellinfo,
                                 const double load);
  double computeFanoutFaninRatio(Vcell *cell);
  double computeFanoutFaninRatioLE(const LibParserCellInfo &cellinfo,
                                   double load);
  double computeFanoutFaninRatioLE(Vcell *cell);
  double computeFanoutFaninRatioLEPosContest(const LibParserCellInfo &cellinfo,
                                             double load);
  ...
  void walkThroughWorstSlackPath(Vcell *seed, EdgeType edgeType,
  ...
  // Walk from primary output and flip-flops to temporal barriers (or
  // vice-versa).
  ...
  // Walk from largest load capacitance to smallest load capacitance and
  // smallest to largest
  ...
  void walkFollowingBacktrackPointers(const int netIndex,
  ...
  struct StepperAssignSizeLogicalEffortFanout {
    ...
    StepperAssignSizeLogicalEffortFanout(Circuit *const circuit)
        : super(circuit) {
      fanoutPerStage = 0;
      fanoutEffortPerStage = 0;
    } // end constructor
  ...
  static double lookupDelay(const LibParserTimingInfo &timingInfo,
                            const EdgeType edgeType, const double inputSlew,
                            const double loadCapacitance);
  static double lookupOutputSlew(const LibParserTimingInfo &timingInfo,
                                 const EdgeType edgeType,
                                 const double inputSlew,
                                 const double loadCapacitance);
  ...
  static void computeArcTiming(const LibParserTimingInfo &timingInfo,
  ...
  void computeArcTimingDelayAndSlewSensitivityToOutputLoad(
      const int k, const int size, EdgeArray<double> &sensitivityDelay,
      EdgeArray<double> &sensitivitySlew);
  void computeArcTimingDelayAndSlewSensitivityToInputSlew(
      const int k, const int size, EdgeArray<double> &sensitivityDelay,
      EdgeArray<double> &sensitivitySlew);
  ...
  static bool nearZero(const double v, const double precision = EPSILON) {
  ...
  static bool approximatelyZero(const T a, const T precision = EPSILON) {
    return std::abs(a) <= ((std::abs(a) < 0 ? 0 : std::abs(a) * precision));
  } // end method
  ...
  const TimingArcState &getTimingArcState(const int k) const {
    return timingStateCurrent.arcs[k];
  }
  const TimingNetState &getTimingNetState(const int n) const {
    return timingStateCurrent.nets[n];
  }

  const TimingArcState &getTimingArcState(const int k,
                                          const State &state) const {
    return state.arcs[k];
  }
  const TimingNetState &getTimingNetState(const int n,
                                          const State &state) const {
    return state.nets[n];
  }

  TimingArcState &getTimingArcState(const int k) {
    return timingStateCurrent.arcs[k];
  }
  TimingNetState &getTimingNetState(const int n) {
    return timingStateCurrent.nets[n];
  }

  TimingArcState &getTimingArcState(const int k, State &state) {
    return state.arcs[k];
  }
  TimingNetState &getTimingNetState(const int n, State &state) {
    return state.nets[n];
  }
  ...
  double getWorstSlack() const {
    return min(this->sdcInfos.clk_period - timingWorstArrivalTime[RISE],
  ...
  EdgeArray<double> getNetSlack(const int n) const {
    return timingRequiredTime[n] - getTimingNetState(n).arrivalTime;
  }
  EdgeArray<double> getNetNegativeSlack(const int n) const {
    return min(EdgeArray<double>(0, 0),
               timingRequiredTime[n] - getTimingNetState(n).arrivalTime);
  }
  ...
  EdgeArray<double> getNetArrivalTimeStandardDeviation(const int n) const {
    ...
    for (int k = k0; k < k1; k++) {
      ...
      const EdgeArray<double> arrivalTime =
          (arc.arrivalTime + arc.delay.getReversed());
      x1 += arrivalTime;
  ...
  const vector<pair<string, string>> &getBestSolution() const {
    return bestSolution;
  }
  ...
  double getLibPinCapacitance(const int footprint, const int type,
                              const int pin) {
  ...
  string getLibPinName(const TimingArc &arc) {
    return (arc.pin >= 0) ? arc.cell->actualInstType->pins[arc.pin].name
                          : "<null>";
  } // end method
  ...
  double getLoadViolation(Vcell *cell) {
    ...
    return (cell->actualLoad > maxLoad) ? (cell->actualLoad - maxLoad) : 0.0;
  } // end method
  ...
  Circuit()
      : maxLeakage(DBL_MAX), maxTimingViol(DBL_MAX), maxWorstSlack(-DBL_MAX),
        maxTransition(0.0), worstSlack(DBL_MAX), worstSlew(0.0),
        worstDelay(0.0), worstNSDelay(0.0), avgDelay(0.0), minSlack(-DBL_MAX),
        minSlew(DBL_MAX), minViol(DBL_MAX), bestCost(DBL_MAX), timingOk(false),
        timingViolationSlew(0), timingTotalNegativeSlack(0), loadViol(0),
        slewViol(0), timingViol(0), accepts(0), rejects(0), acceptsChanges(0),
        rejectsChanges(0), kIndex(0) {
