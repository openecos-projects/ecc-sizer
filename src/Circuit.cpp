...
void Circuit::calcLoadViol() {
  ...
  if (DEBUG_TIAGO)
    cout << " Total Load Violations: " << loadViol << endl;
}
...
void Circuit::readLib() {
  ...
  lib_cells.clear();
  int readCnt = 0;
  for (string lib_file : this->libFiles) {
    LibParser lp(lib_file);
...
void Circuit::sizingLogicalEffort() {
  ...
  for (multimap<double, pair<int, EdgeType>>::reverse_iterator it =
    ...
    stepper2.fanoutPerStage = pow(
        pathFanout,
        1.0 /
            stepper.numPathCells); // can multiply by 2 only for a test, because
...
void Circuit::stepperSlewTarget(Vcell *cell) {
  ...
  //(tmpCell->pinNetPairs[j].first == "d")) ) ) 			tmpCell->pinOk
  //= tmpCell->previousCells.size();
  ...
  //++k) 				if (tmpCell->nextCells[j]->pinNetPairs[k].second
  //== tmpCell->pinNetPairs.back().second)
  //				{
  //					assert(tmpCell->pinNetPairs.back().first
  //== "o");
  //					//if (tmpCell->pinNetPairs.back().first != "o") cerr
  ...
  //=
  // 0.0; 						tmpCell->nextCells[j]->arrivalTimes[k].second =
  // 0.0;
  //					}
  //
  //				}
  ...
  for (i = 0; i < cell->actualInstType->timingArcs.size(); ++i) {
    ...
    if (cell->actualInstType->timingArcs[i].toPin != "o")
      continue;
...
void Circuit::stepperForNoLoadAndSlewViolationByLiLi(Vcell *cell) {
  ...
  for (int i = 0; i < numCandidateCells; i++) {
    ...
    if (meetLoadConstraint && bestLoad == -1)
      bestLoad = i;
    if (meetSlewConstraint && bestSlew == -1)
      bestSlew = i;
...
double
Circuit::computeFanoutFaninRatioLEPosContest(const LibParserCellInfo &cellinfo,
                                             double load) {
...
void Circuit::sizingDepthFanoutXLogicalEffort() {
  ...
  do {
    ...
    iterationA++;
      ...
      (iterationA < 50) &&
      (getWorstSlack() <
       0.0)); //( previousWorstSlack != getWorstSlack() && getWorstSlack() < 0);
...
void Circuit::timingRecovery() {
  ...
  while (true) {
    ...
    for (int i = 0; i < criticalPath.size(); i++) {
      ...
      //) { 					const double score =
      //(oldTNS - timingTotalNegativeSlack) / 					(cell->getLeakagePower() -
      // oldLeakage);
      //
      //					if ( score > bestScore ) {
      //						bestCell = i;
      ...
      if (!cell->dontTouch && !cell->actualInstType->isSequential) {
        ...
        if (increaseVth(cell)) {
          ...
          if (timingTotalNegativeSlack <= currentTNS) {
            ...
            //							<< setw(20) << originalTypeName << "->"
            //<< cell->instType
            //							<< setw(12) << (timingTotalNegativeSlack
            //- currentTNS)
            //							<< setw(10) << currentWS << "->"
...
*/
  ...
  criticalPathDelay +=
      getTimingNetState(n).arrivalTime[edgeType]; // add input driver delay time
                                                  // (when necessary)
...
double
Circuit::updateCellAndComputeSizingEffectOnDriverCellDelay(Vcell *cell,
                                                           const int newSize) {
  //*
...
double
Circuit::computeSizingEffectOnLambdaDelaySensitivities(const int netIndex) {
  EdgeArray<double> effect(0, 0);
      ...
      do {
        ...
        debug("tiago2",
              "load\t\t" << this->loadViol << "\t" << this->totalLeakage << "\t"
                         ...
                         << endl);
      } while (lasLV > this->loadViol * 1.01); // solve max load violations (not
...
double Circuit::slewImpact(TimingArc &arc) { //
  ...
  for (int arcIndex = arc0; arcIndex < arc1; ++arcIndex) {
    ...
    if ((!arc2.cell) || (arc2.cell == arc.cell))
      continue;
